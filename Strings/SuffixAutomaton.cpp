...
struct SuffixAutomaton
{
      ...
      bool contains_pattern(const string &pattern)
      {
            int current_state = 0;
            for (char c : pattern)
            {
                  if (st[current_state].next[c - 'a'] == -1)
                  {
                        return false; // Path broken, pattern doesn't exist
                  }
                  current_state = st[current_state].next[c - 'a']; // Move to the next state
            }
            return true; // Successfully traversed the entire pattern
      }
};