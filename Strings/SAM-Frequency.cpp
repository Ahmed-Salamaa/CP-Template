...
struct SuffixAutomaton
{
      ...
      int get_pattern_frequency( string &s )
      {
            int curr_state = 0 ;
            for ( auto &c : s) 
            {
                  if ( st[curr_state].next[c-'a'] == -1 )
                        return 0 ;
                  curr_state = st[curr_state].next[c-'a'] ;
            }
            return st[curr_state].sz ;
      }
};
...
void solve()
{
      ...
      SuffixAutomaton suff ;
      for ( int i = 0 ; i < s.size() ; i++ )
            suff.extend( s[i] ) ;

      suff.build_frequencies() ;
      suff.build_lifting() ;
      int m ;
      cin >> m ;
      while ( m-- )
      {
            string pattern ;
            cin >> pattern ;
            cout << suff.get_pattern_frequency( pattern ) << '\n' ;
      }
}