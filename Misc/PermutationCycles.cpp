 ...
 * Description: Analyzes an array representing a permutation by decomposing it into disjoint
 *   cycles. Commonly used to compute the minimum number of swaps to sort an array or
...
vector<vector<int>> power(vector<int>& p, int k) {
    ...
    for (auto v : cycles) {
        int len = sz(v), g = __gcd(k, len);  // g cycles of len / g length
        for (int i = 0; i < g; i++) {
            vector<int> w;
            for (int j = i, cnt = 0; cnt < len / g; cnt++, j = (j + k) % len) { w.push_back(v[j]); }
            ans.push_back(w);
        }
    }
    return ans;
