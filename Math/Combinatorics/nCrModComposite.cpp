/*
 * Topic: nCr Modulo Composite
 * Description: Calculates the binomial coefficient nCr modulo a composite number, usually
 *   bypassing large integer arithmetic by using prime factorization of the modulo
 *   and the Chinese Remainder Theorem. Ideal for counting problems with non-prime
 *   constraints.
 *
 * Description:
 * Computes nCr modulo a composite number M using the Chinese Remainder Theorem (CRT) by breaking M into prime powers.
 *
 * How to use it:
 * int ans = nCrModComposite::nCr_mod(n, r, M);
 */
#include "../../core.h"

// Time Complexity: O(M^0.5 + M)
// Space Complexity: O(1)
using int64 = int;
using i128 = __int128_t;

int64 mul_mod(int64 a, int64 b, int64 mod) { return (int64)((i128)a * b % mod); }
...
int64 nCr_mod(int64 n, int64 r, int64 M) {
    ...
    for (size_t i = 1; i < sz(residues); ++i) {
