// Topic: nCr Modulo Any Mod - [Computes nCr modulo arbitrary M]
// NOTE: This template requires basic combinatorics. You must also include Math/Combinatorics/Basics/Combinatorics.cpp.
#include "../../../core.h"

/*
 * Description: An advanced combinatorial algorithm calculating combinations (nCr) modulo any
 *   arbitrary integer, regardless of primality. It combines Lucas' theorem, prime
 *   factorization, and the Chinese Remainder Theorem to handle composite moduli
 *   safely.
 * This mathematical template provides an efficient implementation for nCr Modulo Any Mod.
 * It is commonly used in Advanced Number Theory problems.
 *
 * Usage:
 * nCrModuloAnyMod_Algorithm solver;
 * auto result = solver.power(...);
 */
// Time Complexity: O(P^k)
// Space Complexity: O(P^k)
struct nCrModuloAnyMod_Algorithm {
    const int N = 1e6 + 9;
    using ll = int;
    ...
    int factmod(ll n, int p, const int mod) {
        ...
        while (n > 1) {
            ...
            ans = 1LL * ans * fast_power(f[mod], n / mod, mod) % mod;
            n /= p;
        }
        return ans;
    }
    ...
    int ncr(ll n, ll r, int p, int k) {
        ...
        ans = 1LL * ans * fast_power(p, t, mod) % mod;
        return ans;
    }
