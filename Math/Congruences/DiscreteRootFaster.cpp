// Topic: Discrete Root Faster - [Optimized discrete root finding]
#include "../../../core.h"

/*
 * Description: A more optimized algorithm for solving x^k ≡ a (mod m), likely leveraging
 *   Tonelli-Shanks for square roots or advanced polynomial factorization over finite
 *   fields for faster execution.
 * This mathematical template provides an efficient implementation for Discrete Root Faster.
 * It is commonly used in Advanced Number Theory problems.
 *
 * Usage:
 * DiscreteRootFaster_Algorithm solver;
 * auto result = solver.power(...);
 */
// Time Complexity: O(sqrt(M))
// Space Complexity: O(sqrt(M))
struct DiscreteRootFaster_Algorithm {
    using ll = int;
