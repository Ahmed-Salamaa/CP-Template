// Topic: Pisano Period - [Finds period of Fibonacci sequence modulo M]
#include "../../../core.h"

/*
 * Description: The Pisano Period computes the cycle length of the Fibonacci sequence taken
 *   modulo some integer m. It is used to quickly evaluate huge Fibonacci numbers
 *   under a modulus without computing all the way.
 * This mathematical template provides an efficient implementation for Pisano Period.
 * It is commonly used in Advanced Number Theory problems.
 *
 * Usage:
 * PisanoPeriod_Algorithm solver;
 * auto result = solver.power(...);
 */
// Time Complexity: O(M)
// Space Complexity: O(1)
struct PisanoPeriod_Algorithm {
    const int mod = 1e9 + 7;
    using ll = int;
    ...
    ll pisano_period_prime(ll p) {
        ...
        for (auto [p, e] : mp) {
            ...
            for (int i = 0; i < e; i++) {
                cur *= p;
                for (int j = 0; j < sz; j++) { d.push_back(d[j] * cur); }
            }
        }
        sort(all(d));
        for (auto x : d) {
            if (fib(x, p) == 0 and fib(x + 1, p) == 1) { return x; }
        }
        assert(0);
        return -1;
    }
