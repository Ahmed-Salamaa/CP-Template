 ...
 *   to be transformed into contiguous range queries.
 */
#include "../../core.h"
...
struct DirectedCycle {
    ...
    void dfs1(int s, const std::vector<std::vector<int>>& g, std::vector<char>& vis) {
        std::stack<int> st;
        st.push(s);
        vis[s] = 1;
        while (!st.empty()) {
