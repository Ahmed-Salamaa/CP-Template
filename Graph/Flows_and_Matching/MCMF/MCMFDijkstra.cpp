/*
 * Topic: MCMF Dijkstra
 * Description: Min-Cost Max-Flow using Dijkstra with Potentials (Johnson's technique) to handle
 *   negative edge weights safely in O(F E log V) time.
 * graphs without negative cost cycles. Usage: MCMF<int> mcmf(n); mcmf.add_edge(u, v, cap, cost); auto [flow, cost] =
 * mcmf.solve(s, t);
 */
#include "../../../core.h"
