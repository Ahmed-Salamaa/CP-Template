/* Topic: MO's Algorithm on Trees (Nodes)
 * Description: Adapts Mo's Algorithm for queries concerning node values along the path between
 *   two vertices in a tree. Relies on tree flattening techniques and lowest common
 *   ancestors.
 * Usage: MoTreeNode<int> mo(N, M, G, V); mo.getData();
 */
#include "../../core.h"

template <typename T = int, bool VAL_ON_EDGE = false>
struct MoTreeNode struct MoTreeNode {
    struct Query {
        int l, r, k, lca, queryIdx;
        int64_t ord;

        Query(vector<T>& S, vector<T>& E, int L = 0, int R = 0, int QueryIdx = 0, int LCA = 0, int HilbertPow = 0) {
            if (S[L] > S[R]) swap(L, R);
            if (LCA == L)
                l = S[L] + VAL_ON_EDGE, r = S[R], lca = -1, queryIdx = QueryIdx;
            else
                l = E[L], r = S[R], lca = LCA, queryIdx = QueryIdx;
            calcOrder(HilbertPow);
        }

        void calcOrder(int hilbert_pow) { ord = MoTreeNode::hilbertOrder(l, r, hilbert_pow, 0); }

        bool operator<(const Query& rhs) const { return ord < rhs.ord; }
    };

    int left, right, n, m, SqrtN, timer, LOG, helbertPow;
    T ans;
    vector<T> answers, val;
    vector<int> dep, S, E, FT, nodeFreq;
    vector<vector<int>> adj, anc;
    vector<Query> queries;

    void dfs(int u, int p = -1) {
        S[u] = timer;
        FT[timer++] = u;
        for (auto& v : adj[u]) {
            if (v == p) continue;
            dep[v] = dep[u] + 1;
            anc[v][0] = u;
            for (int bit = 1; bit < LOG; bit++) anc[v][bit] = anc[anc[v][bit - 1]][bit - 1];
            dfs(v, u);
        }
        E[u] = timer;
        FT[timer++] = u;
    }

    void setRange(const Query& q) {
        while (left > q.l) operation(--left);
        while (right < q.r) operation(++right);
        while (left < q.l) operation(left++);
        while (right > q.r) operation(right--);
    }

    void init() {
        // add initialization
    }

    void add(int idx) {
        // add element
    }

    void remove(int idx) {
        // remove element
    }

    inline void operation(int idx) {
        int u = FT[idx];
        nodeFreq[u] ^= 1;
        if (nodeFreq[u] == 1) {
            add(u);
        } else {
            remove(u);
        }
    }

    int calcLog(int max_n) const {
        int log = 0;
        while ((1 << log) <= max_n) log++;
        return log;
    }

    int calcHilbertPow(int max_n) const {
        int pow = 0;
        while ((1 << pow) < max_n) pow++;
        return pow;
    }

    MoTreeNode(int N, int M, vector<vector<int>>& G, vector<T>& V, int root = 1)
        : left(1), right(0), n(N), m(M), SqrtN(n / sqrt(m) + 1), timer(1), ans(0), answers(M), val(V), adj(G) {
        LOG = calcLog(N);
        helbertPow = calcHilbertPow(2 * N + 1);
        nodeFreq = S = E = dep = vector<int>(n + 5);
        FT = vector<int>(2 * n + 5);
        anc = vector<vector<int>>(n + 5, vector<int>(LOG));
        dfs(root);
    }

    static inline int64_t hilbertOrder(int x, int y, int pow, int rotate) {
        if (pow == 0) return 0;
        int hpow = 1 << (pow - 1);
        int seg = (x < hpow) ? ((y < hpow) ? 0 : 3) : ((y < hpow) ? 1 : 2);
        seg = (seg + rotate) & 3;
        const int rotateDelta[4] = {3, 0, 0, 1};
        int nx = x & (x ^ hpow), ny = y & (y ^ hpow);
        int nrot = (rotate + rotateDelta[seg]) & 3;
        int64_t subSquareSize = int64_t(1) << (2 * pow - 2);
        int64_t ordd = seg * subSquareSize;
        int64_t add = hilbertOrder(nx, ny, pow - 1, nrot);
        ordd += (seg == 1 || seg == 2) ? add : (subSquareSize - add - 1);
        return ordd;
    }

    int kthAncestor(int u, int k) const {
        if (dep[u] < k) return -1;
        for (int bit = LOG - 1; bit >= 0; bit--)
            if (k & (1 << bit)) u = anc[u][bit];
        return u;
    }

    int getLCA(int u, int v) const {
        if (dep[u] < dep[v]) swap(u, v);
        u = kthAncestor(u, dep[u] - dep[v]);
        if (u == v) return u;
        for (int bit = LOG - 1; bit >= 0; bit--)
            if (anc[u][bit] != anc[v][bit]) u = anc[u][bit], v = anc[v][bit];
        return anc[u][0];
    }

    void getData() {
        for (int i = 0; i < m; i++) {
            int u, v;
            cin >> u >> v;
            queries.emplace_back(S, E, u, v, i, getLCA(u, v), helbertPow);
        }
        process();
    }

    void process() {
        // Time Complexity: O((N + Q) * sqrt(N))
        sort(all(queries));
        init();
        left = queries[0].l, right = queries[0].l - 1;
        for (const auto& q : queries) {
            setRange(q);
            if (~q.lca && !VAL_ON_EDGE) add(q.lca);
            answers[q.queryIdx] = ans;
            if (~q.lca && !VAL_ON_EDGE) remove(q.lca);
        }
    }

    vector<T> getAnswers() const { return answers; }
};
